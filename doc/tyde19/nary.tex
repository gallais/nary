...
as well as a type-level compositional Domain Specific Language to write clean types involving
...
This paper is a literate Agda file\footnote{The source code is available at
\url{https://github.com/gallais/nary}}; we discuss some of the more
esoteric aspects of the language in Appendix~\ref{appendix:agda}.
...
a level polymorphic inductive family. It has one constructor (\AIC{refl})
witnessing the fact any value is equal to itself.
...
congruence for functions that take more than one argument and
...
or to awkwardly chain the ones with a lower arity to slowly massage
...
underlying data: nil (\AIC{[]}) states that all the elements in the
empty list satisfy \AB{P} and cons (\AIC{\_::\_}) states that \AB{P}
holds of all the elements of a non-empty list if it holds of its head
and of all the elements in its tail.

\ExecuteMetaData[StateOfTheArt.tex]{all}\label{def:all}
...
case of \AR{Σ}; it takes the index \AF{Set} implicitly.
...
The natural counterpart of existential quantification
...
not constructor together with its elimination principle (\AF{⊥-elim}).
...
equivalent of the predicate lifting \AD{All} we have been using as our running example so
...
To state the relational equivalent to \AD{All}'s \AF{\_<⋆>\_} using our
combinators for unary predicates, we need to partially apply \AD{Pw} to
...
the extensions we want to see. Here are the key points we deliver:
...
computing its meaning as a \AF{Set} at some universe level. This level
also needs to be computed from the description.
...
the form ``this type is the result of evaluating the representation of
...
out~\cite{DBLP:conf/tlca/AbelP11}. It is one of the
...
\paragraph{Unification Tests}\label{sec:unificationtest}
We can easily trigger the resolution of
...
Trying to form the unification problem {\AD{ℕ} ≈ (\AB{?A} → \AB{?B})} would
...
is a function type. As explained earlier, Agda communicates to us this failure
to solve the two metavariables passed to \AF{nary} as arguments by
highlighting them in yellow. The constructor \AIC{refl} is also highlighted as
the source of the unification constraint that could not be satisfied.
...
\ExecuteMetaData[StateOfTheArt.tex]{unit}
...
\ExecuteMetaData[StateOfTheArt.tex]{lift}
...
\ExecuteMetaData[Applications.tex]{Subst}
...
various results that fall out naturally from this work. We start with generic
currying and uncurrying, and then use these to define an \AB{n}-ary \AF{zipWith}
...
\ExecuteMetaData[Applications.tex]{zw-aux-type}
...
be finite. Our only principled option is to return the empty list.

\ExecuteMetaData[Applications.tex]{zw-aux0}
...
\ExecuteMetaData[Applications.tex]{zw-aux1}
...
\ExecuteMetaData[Applications.tex]{zw-auxn}
...
First, \AF{Level}s are a first class notion: they can be stored in
...
can for instance explicitly manipulate universe levels.
...
are respectively the predicate and the relational inductive-style translations of
the definition of \AD{List}. The function \AF{replicate} defined in
Section~\ref{par:universalquantifiers}
is a consequence of the abstraction theorem corresponding to the predicate
interpretation and a similar free theorem stating that if a relation is reflexive
then so is its pointwise lifting could have been derived for \AD{Pw}. Bernardy,
Jansson, and Paterson's work on parametricity for dependent
types~\cite{DBLP:journals/jfp/BernardyJP12} makes these observations formal and
...
We would like to thank the reviewers for their helpful comments and their suggestions
to discuss parametricity as a derivation principle, and to add an appendix to make the
paper accessible to a wider audience.

\noindent{}The research leading to these results has received funding from EPSRC grant EP/M016951/1.

\noindent{}All data underpinning this publication are openly available from the University of
Strathclyde KnowledgeBase at \url{https://doi.org/10.15129/7f135ecb-07a6-4a53-aeb5-092c413114ab}.
