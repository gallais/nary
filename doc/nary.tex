...
programming from an arity-generic zipWith to a direct style definition of printf.
...
{(\AB{P} \ARF{proj₁})}. We can construct pairs using \AIC{\_,\_} and we can
project either of the pair's components simply by using its field's name.
...
its two index list have the same length and that they are compatible with \AB{R} in
a pointwise manner.
...
Ideally we could have instead used combinators similar to the ones for unary
predicates and have simply written:
...
representation \AD{Rep} and its semantics \AF{⟦\_⟧} so that a constraint
of the form {\AF{⟦} \AB{?r} \AF{⟧} ≈ (\AD{ℕ} → \AF{Set})} can easily
...
introduced in Section~\ref{def:sigma}. We did not mention at the time but
record types in Agda enjoy η-rules. That is to say that any value \AB{p} of type
{(\AR{Σ} \AB{A} \AB{P})} is equal to {(\ARF{proj₁} \AB{p} \AIC{,} \ARF{proj₂} \AB{p})}
...
of \AF{Product}. We provide both as well as conversion functions between the
two.
...
either a \AIC{Nat} corresponding to a ``\%u'' directive (i.e. unsigned decimal
integer) or a \AIC{Raw} string.
...
are adequately heterogeneous, their approach would not scale to universe
