...
In the following section we write \AB{?a} for a metavariable,
\AB{e}[\AB{?a₁}, ⋯ ,\AB{?aₙ}] for an expression \AB{e} containing exactly the
metavariables \AB{?a₁} to \AB{?aₙ}, and {\AB{lhs} ≈ \AB{rhs}} to state a
unification problem between two expressions \AB{lhs} and \AB{rhs}.
...
under-constrained so it is not surprising that the right-hand side lits up.
