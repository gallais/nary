...
a total language~\cite{DBLP:journals/jucs/ATurner04}: all \AD{List} values
have to be finite. Our only option is to return the empty \AD{List}.
...
{((\AB{A} \AR{×} \AB{A₀} \AR{×} \dots \AR{×} \AB{Aₙ}) → \AB{R})}, a list of \AB{A}s
and a product of lists {(\AD{List} \AB{A₀} \AR{×} \dots \AR{×} \AD{List} \AB{Aₙ})}.
We massage the function to obtain another one of type
{((\AB{A₀} \AR{×} \dots \AR{×} \AB{Aₙ}) → (\AB{A} → \AB{R}))} which we can
