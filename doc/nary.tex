...
In the following section we write \AB{?a} for a metavariable, and
{\AB{lhs} ≈ \AB{rhs}} to state a unification problem between two
expressions \AB{lhs} and \AB{rhs}.

For testing purposes we can easily trigger unification problems in the
source language. We can force Agda to introduce metavariables by using
an underscore (\AF{\_}) as a placeholder for a subterm and use \AIC{refl}
at the proof of a propositional equality to force it to unify the two
expressions stated to be equal. For instance in the following test we
force Agda to check that {(\AB{?A} → \AB{?B}) ≈ (\AD{ℕ} → \AD{ℕ})}

\ExecuteMetaData[Unifier.tex]{unifproblem}

\subsection{Trivial Solution}

Metavariable on one side, expression on the other side not mentioning
this metavariable. Solution by instantiating the metavariable to the
candidate expression.

\subsection{Constructor Headed}

Both expressions are constructor-headed. Either the constructors match
up and we can reduce the problem to unifying their respective arguments.
Or they do not and we can conclude straight away: unification is impossible.
...
type. Agda communicates to us this failure to solve the two metavariables
passed to \AF{nary} as arguments by highlighting them in yellow, \AIC{refl}
is also highlighted as the source of the unification constraint which
...
\section{Combinators for Indexed Families}
...
\todo{Discuss non top terminated}
...
and produces a list by processing the lists in lockstep combining their
elements thanks to the function it was passed. Using ellipses, we would
write its type as:

\medskip
\noindent\begin{tabular}{@{}l@{~}l}
  \AF{zipWithₙ} : & (\AB{A₁} → ⋯ → \AB{Aₙ} → \AB{B}) →\\
...
(un)currying in the appropriate places. This is the function we want:

\ExecuteMetaData[N-ary.tex]{zipWith}

But this is how we implement it:

\ExecuteMetaData[N-ary.tex]{zw-aux}

\subsection{Printf}

We can implement \AF{Printf} in direct style, recovering the usual
type by generic currying.
...
Telescopes rather than mere \AF{Sets}.
